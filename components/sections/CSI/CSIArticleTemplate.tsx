...
export default function CSIArticleTemplate({ slug }: { slug: string }) {
    ...
    const initiativeIndex = useMemo(() => initiatives.findIndex(i => i.slug === `/${slug}`), [slug]);
    const initiative = initiatives[initiativeIndex];

    // Find the complex article data (paragraphs, galleries, etc)
    const articleData = useMemo(() => csiArticlesData.find(a => a.slug === `/${slug}`), [slug]);
    ...
    const prevIndex = initiativeIndex > 0 ? initiativeIndex - 1 : initiatives.length - 1;
    const nextIndex = initiativeIndex < initiatives.length - 1 ? initiativeIndex + 1 : 0;
        ...
        >
            {articleData.sections.map((section, idx) => {
                switch (section.type) {
                    ...
                    case 'highlight':
                        return (
                            <div key={idx} className="my-10 p-8 bg-gray-50 dark:bg-darklight border-l-4 border-primary rounded-r-xl italic text-lg text-gray-700 dark:text-gray-300">
                                {section.text}
                            </div>
                        );
                    case 'heading':
